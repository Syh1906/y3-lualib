...
本文记录 y3-lualib 的用户可见变化。新条目按“双轨”维护：上游同步记录来自上游基准，本地迭代记录来自当前分支维护。
...
- 新增双版本机制，区分上游基准版本与本地迭代版本。

### 兼容影响

- `y3.version` 继续表示上游基准版本。
- 新增 `y3.release`、`y3.upstream_version`、`y3.local_version`、`y3.full_version`。

## moy-2026.07.03.1 - 2026-07-03

### 上游基准

- 基于上游版本号 `260616`，当前基准提交记录在 `release.lua`。

### 本地迭代

- 建立本地分支版本号 `moy-2026.07.03.1`。

## 历史记录

以下内容保留原有日期记录，后续新变更写入上方双轨结构。

`2025-8-7`
+ 使用 `Unit:add_buff` 时传入的 `data` 字段可以使用 `Buff.data` 来访问
+ 添加接口：
  * `Buff:set_icon` 设置魔法效果的图标

`2025-8-4`
+ 修复了 `y3.sync.send` 同步的数据中包含稀疏数组时可能会丢失部分数据的问题
